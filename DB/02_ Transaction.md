...
- 즉, 커밋되지 않은 데이터조차 접근할 수 있는 격리 수준
...
- 즉, 다른 트랜잭션이 커밋한 데이터만 읽을 수 있다.
- DIRTY READ 현상 발생 X
- Non-repeatable Read 현상 발생
   - **Non-repeatable Read**: 한 트랜잭션에서 같은 쿼리를 두 번 실행할 때 그 사이에 다른 트랜잭션 값을 수정 또는 삭제하면서 두 쿼리의 결과가 상이하게 나타나는 일관성이 깨진 현상 (같은 트랜잭션 안에서 SELECT 결과가 시점에 따라 달라지는 현상)
...
- 한 번 읽은 데이터 다시 읽어도 같은 결과가 나온다. 
- 같은 트랜잭션 범위 내에서는 SELECT 결과가 변하지 않는다.
