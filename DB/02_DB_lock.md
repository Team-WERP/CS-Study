...
- **`SELECT … LOCK IN SHARE MODE`** 같은 쿼리에서 명시적으로 사용
...
쓰기 작업은 데이터의 상태를 변경하므로, 동시에 접근이 허용되면 데이터 일관성과 무결성이 즉시 깨짐.

배타락은 **데이터 변경의 안전성**을 보장하는 핵심 메커니즘이다.
...
| **LOCK IN SHARE MODE** | SELECT 시 공유락(S-Lock)을 명시적으로 요청하는 SQL 구문. 읽기는 허용하지만 수정은 차단한다. |
