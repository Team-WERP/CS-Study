...
**다운캐스팅**<br/>
부모 타입(Parent) 변수로 참조하고 있는 객체를 자식 타입(Child)으로 다시 바꿔서(캐스팅해서) 자식만의 기능(메서드나 필드)을 사용할 수 있게 하는 것.<br/>

Parent 타입 변수로는 Parent가 가진 기능만 사용할 수 있음
```
Parent poly = new Child();
poly.parentMethod();   // OK
poly.childMethod(); //  컴파일 에러
```
위와 같은 경우에서 자식 메서드를 사용하려면 
```
Parent poly = new Child();
